...
with tabs[0]:
    ...
    if st.button("Generate Periodic Table Graph"):
...
with tabs[1]:
    ...
    if st.button("Generate Instability Graph"):
...
with tabs[2]:
    ...
    if st.button("Generate Star Formation Graph"):
...
with tabs[3]:
    ...
    if st.button("Generate Life Probability Map"):
        strong_force_values = np.linspace(0.1, 10.0, 50)
        em_force_values = np.linspace(0.1, 10.0, 50)
...
with tabs[4]:
    ...
    if st.button("Generate Quantum Bonding Graph"):
        strong_force_values = np.linspace(0.1, 10.0, 50)
        em_force_values = np.linspace(0.1, 10.0, 50)
        strong_grid, em_grid = np.meshgrid(strong_force_values, em_force_values)
...
with tabs[9]:
    ...
    if st.button("Generate Dark Matter Map"):
...
with tabs[10]:
    ...
    if st.button("Generate Atomic Stability 3D Graph"):
        atomic_numbers = np.arange(1, 121)
